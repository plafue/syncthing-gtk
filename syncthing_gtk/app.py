...
class App(Gtk.Application, TimerManager):
    ...
    def cb_syncthing_error(self, daemon, message):
        ...
        if "remote device speaks an older version of the protocol" in message:
            ...
            folder_id = re.search("to ([-0-9A-Za-z]+)", message)
            version = re.search("protocol \(([^\)]+)\)", message)
            if not folder_id or not version:
                # Invalid format
                return
            folder_id = display_id = folder_id.group(1)
            version = version.group(1)
            if folder_id in self.devices:
                device = self.devices[folder_id]
    ...
    def show_device(self, folder_id, name, compression, introducer, used):
        if name in (None, ""):
            # Show first block from ID if name is unset
            name = folder_id.split("-")[0]
        if not used:
            name = "%s (%s)" % (name, _("Unused"))
        if folder_id in self.devices:
            # Reuse existing box
            box = self.devices[folder_id]
            box.set_title(name)
        else:
            # Create new box
            box = InfoBox(self, name, IdentIcon(folder_id))
            # Add visible lines
            ...
            box.add_value('last-seen',	"clock.svg",
                          _("Last Seen"),			_("Never"))
            # Add hidden stuff
            box.add_hidden_value("id", folder_id)
            ...
            box.set_vexpand(False)
            box.set_open(folder_id in self.open_boxes)
